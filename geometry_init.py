...
def generate_plane(width, height, MAX_WIDTH_SIZE=500, MAX_HEIGHT_SIZE=300):
    ...
    add_fixed_constraint(n, 0, verts[0], constraints)
    bottom_left = width * (height - 1)
    add_fixed_constraint(
        n, bottom_left, verts[bottom_left], constraints)
...
def add_fixed_constraint(number_of_verts, v_id, vert, constraints):
    constraints.append(constraint.Constraint(
        number_of_verts, v_id, fixed_point=vert))

def add_spring_constraint_set(verts, v_1, v_2, v_3, constraints):
    add_spring_constraint(verts, v_1, v_2, constraints)
    add_spring_constraint(verts, v_2, v_3, constraints)
    add_spring_constraint(verts, v_3, v_1, constraints)

def add_spring_constraint(verts, v_1, v_2, constraints):
    rest_length = linalg.norm(verts[v_1] - verts[v_2])
    number_of_verts = len(verts)
    constraints.append(constraint.Constraint(
        number_of_verts, v_1, v_2, rest_length=rest_length))